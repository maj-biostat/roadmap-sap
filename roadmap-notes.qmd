...
Previously this section included a statement on how we should interpret missing elements in the composite as a failure case (or alternative approach required) but want to know (1) how we should be handling this and (2) how likely it is to happen.

The core protocol, see section "6.6 Trial endpoints" does not indicate what happens when elements of the composite cannot be identified. It probably should make some statement regarding how missingness is dealt with from an operational perspective or in terms of the impact on interpretation.
...
"Within each relevant cell" is probably redundant.
...
Move treatment success from domain specific to domain agnostic analyses and assumed to replicate the primary analysis with the exception of using a per protocol population (per protocol across all domains is what has been assumed).
...
*The estimand in the core protocol assumes a cumulative logit analysis whereas here we assume a 'classical' door analysis.*
...
# Section 2.9.6 - Clinical cure to 12 months

Update the estimand measure of effect to a risk difference.

# Section 2.9.7 - Clinical cure to 12 months

Or is it the intention to restrict this to the subset that don't die and don't get their initial prosthetic replaced.
...
Get the DSA estimand specifications update to indicate that these are all up to 12 months.
...
Specifically, what is with the other prefix? This applies to the equivalent estimand in the extended proph domain.
