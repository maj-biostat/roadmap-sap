...
The main objective of the ROADMAP trial is to examine the effects associated with a range of interventions in patients with PJI (defined by the eligibility criteria) on treatment success where this is defined as the patient being alive with clinical cure, not taking antibiotics for PJI and with the destination prothesis in place and is assessed at 12 months.
...
See estimand A.1 in the master protocol, which has been extended here in order to provide additional details on all the comparisons that are involved.

A.1 approximates a *"pure-ITT"* approach in that all ICEs are intended to be handled under a treatment policy strategy whereby the ICEs are effectively considered to be part of the treatment regimen of interest and can therefore be ignored @Mallinckrodt2020.
We do, however, note that the definition of the primary outcome implies a composite strategy for both the ICEs relating to death and the replacement of desination prothesis.

The additional detail provided here is intended to aid thinking and interpretation as the use of a treatment policy strategy has particular implications, especially when non-inferiority is a quantity of interest.
...
These definitions give the full detail on the setting and what needs to occur for each interventions in order for it to be considered to be per-protocol.
For example, at a minimum, debridement and implant retention (DAIR) must include an open approach, exchange of modular components.
...
::: {.callout-note}
Other ICEs are probably relevant here.
...
Under a treatment policy strategy, the treatment regimen under consideration is, in fact, the initial randomized treatment with discontinuation or switch of treatment and/or alterations in background therapy, changes in medication and rescue therapy at any point in time.
...
Under the above assumptions, that is the missingness is not simultaneously dependent on the outcome and exposure, imputation is not technically required and a complete-case analysis will yield unbiased estimates of the treatment effects.
Therefore, imputation will not be adopted for the primary outcome variable.
...
Tipping point sensitivity analysis and multiple imputation may be considered and implemented at the discretion of the analyst.
...
Extended therapy in the 6 week group (A.1.2(i1)) would make the responses more similar to the 12 week group leading to increase likelihood of non-inferiority decision, i.e. conclusions may anti-conservative. 
Similarly, early termination in the 12 week group (A.1.2(i2)) would make the responses more similar to the 6 week group.
...
d_tbl <- tribble(
  ~ id, ~ arm, ~ ice, ~ class , ~ strategy, 
  "A.1.3(i1)" , "None"  , "If the patient has slow improvement, recrudescence of infection or need to return to theatre then clinicians may choose to prolong extended prophylaxis therapy." , "Extended therapy" , "Treatment policy" ,
"A.1.3(i2)" , "None"  , "If the patient shows late positive culture, they may become ineligible after revealed to their and would also receive an additional 6-12 weeks of backbone antibiotic." , "Extended therapy" , "Treatment policy" ,
...
Extended therapy in the no extended prophylaxis group would make the group more similar to the 12 week group leading to increase likelihood of non-inferiority decision, i.e. conclusions may anti-conservative. 
Equally, early termination in the 12 week group would make the responses more similar to the no extended prophylaxis group.
...
::: {.callout-note}
The DSA considers some secondary estimands, e.g. the surgical domain (late-actue) outlines an interest per protocol estimand.
TODO - revisit and introduce necessary analysis specifications.
:::
...
The minimal requirements for retaining patients within the per-protocol population set are provided in @tbl-pp1.
...
The DOOR method involves comparing trial participant outcomes in different treatment arms by the desirability of the overall patient outcome.
The desirability of outcome ranking (DOOR) criteria for ordering outcomes are provided in @tbl-sec-door.
From this, each patient is assigned a ranking, which is simply an ordinal random variable, to characterise their state.
...
gt_tbl <- gt(d_tbl) |>
  ...
  cols_label(
   ...
   tab_footnote(
    footnote = md("Treatment Success relates to primary outcome"),
...
The analysis approach targets a what has been called a DOOR probability, but is more broadly referred to as a Probabilistic Index (PI) @DeSchryver2018.
Pairwise comparisons between all patients are used to compute the PI as
...
where $n_{win}$ and $n_{tie}$ are the number of instances where the experimental units do *better* than the control patients or have equivalent ranking respective for all pairs and $n_e$ and $n_c$ are the number of patients in each of the two arms being compared.
Confidence intervals are then computed via a bootstrap procedure or other methods.

The PI refers to the probability that the outcome of a randomly selected subject in one group exceeds the outcome of another randomly selected subject in another group (plus half of the probability of a tied DOOR) @Evans2015.
...
For further cautions, see Senn @Senn2011.
:::

Separate DOOR analyses will be run on relevant strata based on the assessment and ranking of the relevant outcomes at 12-months:
...
As of 2007, responses range from 0 to 4 and the total score has a maximum (best) value of 48 with 40-48 indicating satisfactory joint function and 3-5 being a suggested clinically important difference.

The OHS will be analysed using cumulative logistic regression.
...
and where the (generically stated) linear predictor would have terms and priors analogous to those used in the primary analysis.
...
::: {.callout-note}
TODO - generally, the QoL analyses rely on utility values to produce QALYs for a cost-utility assessment. 
These are ordinarily underkane by an econometrician.
:::

### Cost effectiveness

While a cost-effectiveness analysis will likely use Bayesian simulation, this work will be undertaken by the econometrician and is beyond the scope of the specification here.

### All-cause mortality at 12 months after platform entry {#sec-all-cause-mortality}

We will use a proportional-hazards model to analyse all-cause mortality at 12-months.
...
We will use a proportional-hazards model (per @sec-all-cause-mortality) to analyse clinical cure at 12-months.

### Proportion no longer taking any antibiotics for the index joint at 12 months (as defined in the primary outcome) {#sec-prop-ab-free}

Recalling the primary outcome:
...
this secondary analysis aims to characterise the proportion of patients no longer taking antibiotics for the index joint at 12-months.
Therefore, this is actually a comparison of no-longer taking antibiotics versus taking antibiotics or death.
...
### Proportion in whom the destination prosthesis is still in place at 12 months (as defined in the primary outcome) {#sec-joint-in-place}

As detailed in @sec-prop-ab-free, this is actually a comparison of having the destination prosthesis in place versus not having the destination prosthesis in place or death.

The same approach as detailed in @sec-prop-ab-free will be used.

### Microbiological relapse (Positive joint fluid or tissue culture for one or more of the index isolates between 100 days and 12 months after platform entry) {#sec-microbiological-relapse}

The occurrence of microbiological relapse is subject to the competing risk of death.
...
# about 60% resistant but with site and subject level variability beyond the
# natural sampling variability due to varying number of subjects per site
nu <- rnorm(N, 0, 0.5)
# treat site as true fixed effect
rho <- rnorm(N_site, 0, 0.7)
d[, eta := 0.4 + rho[site] + nu[id_pt]]

# hist(d[, unique(eta), keyby = id_pt][,plogis(V1)])

d[, y := rbinom(.N, 1, plogis(eta))]
d[, site := factor(site)]
d[, id_pt := factor(id_pt)]

# summary stats - we got much more data here than you do but this is just for
# demo
d[, .(y = sum(y), n = .N)]
...
# sample proportion (same as model estimate)
p_obs <- d[, sum(y)/.N]
p_obs
