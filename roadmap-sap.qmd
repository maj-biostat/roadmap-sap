...
Missing data is possible due to inability to identify clinical records, formal requests to completely withdraw from the study (including use of data), loss to follow up due to relocation.
...
Missingness is assumed to be at random across all variables and variable components.
This does not bias treatment effect estimation for logistic regression so long as the missingness is not dependent on the outcome and exposure, imply validity of complete-case analysis.
