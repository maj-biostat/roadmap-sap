...
The primarily application is to inform clinical decision making at the point of care.
...
The absence of any of the above elements is considered to be a treatment failure for the purposes here.
...
They need more detail than they presently have but we can also flesh out further here.
@Mallinckrodt2020 gives guidanace on how to improve the current specification. 
The way the estimands are presently stated is tokenistic, they don't provide any real value.
:::
...
Under the above assumptions, that is the missingness is not simultaneously dependent on the outcome and exposure, multiple imputation is not technically required to obtain unbiased estimates of the treatment effect.
...
This is despite the fact that I believe that the conditional perspective is the only one that really makes sense here in terms of being reasonably well defined and that the inherently vague pragmatic perspective is not the right way to approach this research problem.
:::
...
Other sections are as defined for B.1.
...
I am aware that views differ on this and we need to discuss.
:::
...
Thus, the sample size is initially constrained by the current resources and the desired structure.
...
Notwithstanding the clinicial team and patients involved in delivery and receipt of the interventions, only the analytical and data groups responsible for providing analysis results to the DSMC will be privy to individual treatment group assignments.
...
::: {.callout-note}
We note that a Bayesian approach to the MWU exists but has not been widely adopted, @Chechile2020.
:::
...
todo - go back and review M-spline vs B-spline differences in approach, stability and so on.
:::
...
Specifically, assuming no time-varying covariates or coefficients, for risk $r$ we have:
...
The approach assumes independent censoring for patients who are not truly censored but that fail fail from the competing event, most commonly death.
This means that any censored patient at time $t$ would have the same rate of clinical cure, regardless of whether the censoring was due to administrative censoring or any of the other event types.
There is no way to explicitly test this assumption.

::: {.callout-note}
todo - would a multi-state model perspective be more appropriate for these?
:::
...
Therefore, this will be analysed via a cause-specific hazard survival model, analogous approach as that detailed in @sec-clinical-cure.
...
Time alive and free from any revision procedure on the index joint captured by a national joint replacement registry within 12 months of domain entry is subject to the competing risk of death.
Therefore, this will be analysed via a cause-specific hazard survival model, analogous approach as that detailed in @sec-clinical-cure.
...
Time alive and free from any revision procedure on the index joint captured by a national joint replacement registry within 24 months of domain entry is subject to the competing risk of death.
Therefore, this will be analysed via a cause-specific hazard survival model, analogous approach as that detailed in @sec-clinical-cure.
...
However, any occurrence of a futility decision requires a subgroup analysis be performed as per the populations listed in @sec-subgroups using the methods detailed in @sec-subgroup-analyses for the relevant domain(s).
