...
\text{logit}(p) &= \mu + \lambda_s + \rho_j + \phi_q + \sum_{d \in \mathcal{D}} \va{x}_{d}^\prime \va*{\beta}_{d} + \psi_r + \zeta_{z(r)} + \tau_t + \va{w}^\prime \va*{\gamma}
\end{aligned}
$$
...
+	$\va*{\beta}_{d}$ shift attributable to each vector of treatment effects (including non-participation effects) as indicated by the row vector $\va{x}$ for domain $d$ in the set of available domains $\mathcal{D}$ 
...
$$
(\lambda_s, \rho_j, \phi_q, \va*{\beta}_d, \va*{\gamma}) \sim \text{Normal}(0, 2)
$$
...
# Statistical quantities {#sec-statistical-quantities}
...
Trial decisions are made based on probability assessments with respect to comparisons of interest that are characterised by model parameters as discussed earlier.
